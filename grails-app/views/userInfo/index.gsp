		...
		<div id="list-userInfo" class="content scaffold-list" role="main">
			...
			<thead>
					<tr>
						...
						<g:sortableColumn property="nickName" title="${message(code: 'userInfo.nickName.label', default: 'Nick Name')}" />
					
						<g:sortableColumn property="iconURL" title="${message(code: 'userInfo.iconURL.label', default: 'Icon URL')}" />
				...
				<g:each in="${userInfoInstanceList}" status="i" var="userInfoInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
						...
						<td>${fieldValue(bean: userInfoInstance, field: "nickName")}</td>
					
						<td>${fieldValue(bean: userInfoInstance, field: "iconURL")}</td>
